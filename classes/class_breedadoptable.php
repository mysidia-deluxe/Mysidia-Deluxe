...
class BreedAdoptable extends Adoptable{
    ...
    public function getParent($fetchMode = ""){
	    if($fetchMode == Model::MODEL) return new Adoptable($this->parent);
        else return $this->parent;
    }
	
    public function getMother($fetchMode = ""){
	    if($fetchMode == Model::MODEL) return new Adoptable($this->mother);
        else return $this->mother;
    }	
  
    public function getFather($fetchMode = ""){
	    if($fetchMode == Model::MODEL) return new Adoptable($this->father);
