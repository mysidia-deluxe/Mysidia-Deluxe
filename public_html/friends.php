...
class FriendsController extends AppController
{
    ...
    public function request()
    {
        ...
        if (!$friend->isfriend) {
            if ($friend->sendrequest()) {
                $this->setField("friend", new Mystring($friend->username));
            } else {
                throw new DuplicateIDException("<br>Invalid Action! This is a duplicate friend request between you and {$friend->username}.");
            }
        } else {
            throw new InvalidIDException("<br>Invalid Action! The user {$friend->username} is already on your friendlist.");
        }
    }
