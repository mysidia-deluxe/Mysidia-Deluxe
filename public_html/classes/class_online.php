...
class Online extends UserContainer
{
    ...
    public function update()
    {
        ...
        if (!$userexist) {
            $mysidia->db->insert("online", array("username" => $username, "ip" => $ip, "session" => $session, "time" => $currenttime)) or die('Cannot insert user data.');
        } else {
            $mysidia->db->update("online", array("time" => $currenttime, "session" => $session, "username" => $username), "username = '{$username}' and ip = '{$ip}'");
        }
