...
class Itemshop extends Model
{
    ...
    {
        $mysidia = Registry::get("mysidia");
        if ($item->shop != $this->shopname) {
            throw new NoPermissionException('Did you really think this item could be bought at this shop?');
        } else {
            if ($item->owner != $mysidia->user->username) {
