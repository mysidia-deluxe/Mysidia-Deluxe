...
class AccessoryRenderer extends GUIRenderer
{
    ...
    public function __construct(GUIAccessory $accessory)
    {
        ...
        if ($accessory instanceof Link) {
            $this->tag = "a";
        } elseif ($accessory instanceof Image) {
            $this->tag = "img";
        } elseif ($accessory instanceof Media) {
            $this->tag = "object";
        } else {
            $this->tag = strtolower(get_class($accessory));
        }
    }
    ...
    {
        if ($this->component->getFor()) {
            $this->setRender(" for='{$this->component->getFor()}'>");
        } else {
            $this->setRender(">");
        }
        return $this;
