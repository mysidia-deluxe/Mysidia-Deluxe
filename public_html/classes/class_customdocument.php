...
class CustomDocument extends Widget implements Initializable
{
    ...
    */
    ...
    public function __construct($document = "")
    {
        ...
        if (!is_object($page)) {
            throw new PageNotFoundException($mysidia->lang->nonexist);
        }
    ...
    protected function getAccess()
    {
        ...
        if ($this->code) {
            if ($mysidia->input->post("promocode")) {
                ...
                if ($this->code != $mysidia->input->post("promocode")) {
                    throw new NoPermissionException($mysidia->lang->wrongcode);
                }
           
                $promo = new Promocode($mysidia->input->post("promocode"));
                if ($promo->pid == 0 or !$promo->validate($mysidia->input->post("promocode"))) {
                    throw new NoPermissionException($mysidia->lang->nocode);
                }
        ...
        if ($this->item) {
            if ($mysidia->input->post("item")) {
                ...
                if ($mysidia->input->post("item") != $this->item) {
                    throw new NoPermissionException($mysidia->lang->wrongitem);
                }
           
                $item = new PrivateItem($this->item, $mysidia->user->username);
                if ($item->iid == 0 or $item->quantity < 1) {
                    throw new NoPermissionException($mysidia->lang->noitem);
                }
        ...
        if ($this->time) {
            ...
            $current = time();
            if ($current < $time) {
                throw new NoPermissionException($mysidia->lang->wrongtime);
            }
        }
        if ($this->group) {
            $group = $mysidia->usergroup->gid;
            if ($group != $this->group) {
                throw new NoPermissionException($mysidia->lang->notgroup);
            }
        }
        return true;
    }
