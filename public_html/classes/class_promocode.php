...
class Promocode
{
    ...
    public function __construct($code = "")
    {
        ...
        if ($row = $stmt->fetchObject()) {
            ...
            if (empty($this->user)) {
                $this->user = $mysidia->user->username;
            }
        } else {
            return false;
        }
    ...
    private function checkuser($user = "")
    {
        ...
        if (empty($this->user) or $this->user == "SYSTEM" or $user == $this->user) {
            $uservalid = true;
        } else {
    ...
    private function checkdate()
    {
        ...
        $mysidia = Registry::get("mysidia");
        $currenttime = time();
        if (empty($this->fromdate) and empty($this->todate)) {
            return true;
        } else {
    ...
    public function execute()
    {
        ...
        if ($this->valid != true) {
            throw new NoPermissionException($mysidia->lang->validate);
        }
    ...
    private function usepromo()
    {
        $mysidia = Registry::get("mysidia");
        if ($this->availability == 0) {
            throw new NoPermissionException($mysidia->lang->fatalerror);
        }
        $this->availability--;
