...
class AdoptConditions extends Model
{
    ...
    protected function checkFreqCondition()
    {
        if ($this->freqcond == "enabled") {
            ...
            if ($freq >= $this->number) {
                throw new AdoptConditionsException("Freq Condition Not met.");
            }
        }
    }
    ...
    protected function checkDateCondition()
    {
        if ($this->datecond == "enabled") {
            ...
            if ($this->date != $today->format('Y-m-d')) {
                throw new AdoptConditionsException("Date Condition Not met.");
            }
        }
    }
    ...
    protected function checkNumberCondition()
    {
        if ($this->moreless == "enabled") {
            ...
            if ($num >= $this->morelessnum) {
                throw new AdoptConditionsException("Number Condition Not met.");
            }
        }
    }
    ...
    protected function checkGroupCondition()
    {
        if ($this->levelgrle == "enabled") {
            $mysidia = Registry::get("mysidia");
            if ($mysidia->user->usergroup->gid != $this->grlelevel) {
                throw new AdoptConditionsException("Group Condition Not met.");
            }
        }
    }
    ...
    public function checkConditions()
    {
        ...
        if (cando($mysidia->user->usergroup, "canadopt") != "yes") {
            return false;
        }
