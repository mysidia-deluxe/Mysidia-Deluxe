...
class AccountController extends AppController
{
    ...
    public function password()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if ($currentpass != $userdata->password) {
                throw new PasswordException("password_current");
            } elseif ($newpass1 != $newpass2) {
                throw new PasswordException("password_new");
            } elseif (!$mysidia->input->post("np1") or !$mysidia->input->post("np2")) {
                throw new PasswordException("password_blank");
            } else {
    ...
    public function profile()
    {
        ...
        if (!($profile instanceof UserProfile)) {
            throw new ProfileException("profile_nonexist");
        } elseif ($mysidia->user->uid != $profile->uid) {
            throw new ProfileException("profile_edit");
        } else {
