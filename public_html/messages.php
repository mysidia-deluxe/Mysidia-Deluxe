...
class MessagesController extends AppController
{
    ...
    {
        $mysidia = Registry::get("mysidia");
        if (!empty($recipient->friends)) {
            $friends = explode(",", $recipient->friends);
        }
        $isfriend = (empty($recipient->friends) or !in_array($mysidia->user->uid, $friends))?"no":"yes";
        if ($recipient->options->pmstatus == 1 and $isfriend == "no") {
            throw new InvalidActionException("error_friend");
        }
        if (!$mysidia->input->post("mtitle") or !$mysidia->input->post("mtext")) {
            throw new InvalidActionException("error_blank");
        }
        if ($mysidia->input->post("outbox") == "yes" and $mysidia->input->post("draft")) {
