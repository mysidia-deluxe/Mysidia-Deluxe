...
class MytradesController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($stmt->rowCount() == 0) {
            throw new InvalidIDException($mysidia->lang->empty);
        }
    ...
    {
        $mysidia = Registry::get("mysidia");
        if (!(string)$mysidia->input->get("tid")) {
            throw new TradeNotfoundException("accept_none");
        }
    ...
    public function decline()
    {
        $mysidia = Registry::get("mysidia");
        if (!(string)$mysidia->input->get("tid")) {
            throw new TradeNotfoundException("decline_none");
        }
        if ((string)$mysidia->input->get("confirm")) {
            if (!$mysidia->session->fetch("tid")) {
                die("Session already expired");
