...
function __autoload($name)
{
    ...
    $dir = (defined("SUBDIR"))?"../":"";
    if (file_exist("{$dir}{$class_path}.php")) {
        include("{$dir}{$class_path}.php");
    } else {
        $abstract_path = strtolower("classes/abstract/abstract_{$class}");
        $interface_path = strtolower("classes/interfaces/interface_{$class}");
        if (file_exist("{$dir}{$abstract_path}.php")) {
            include("{$dir}{$abstract_path}.php");
        } elseif (file_exist("{$dir}{$interface_path}.php")) {
            include("{$dir}{$interface_path}.php");
        } else {
            throw new Exception("Fatal Error: Class {$class} either does not exist!");
        }
    }
}
...
function secure($data)
{
    ...
    if (is_array($data) and SUBDIR != "AdminCP") {
        die("Hacking Attempt!");
    }
