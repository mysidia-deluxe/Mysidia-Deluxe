        ...
        if ($mysidia->input->post("submit")) {
            ...
            foreach ($settings as $name) {
                if ($mysidia->input->post($name) != ($mysidia->settings->{$name})) {
                    $mysidia->db->update("settings", array("value" => $mysidia->input->post($name)), "name='{$name}'");
                }
            }
        }
    }
    ...
    public function pound()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if ($mysidia->input->post("costtype")) {
                $mysidia->db->update("pound_settings", array("advanced" => $mysidia->input->post("costtype")), "varname='cost'");
            }
            if ($mysidia->input->post("leveltype")) {
                $mysidia->db->update("pound_settings", array("advanced" => $mysidia->input->post("leveltype")), "varname='levelbonus'");
            }
            if ($mysidia->input->post("dateunit")) {
                $mysidia->db->update("pound_settings", array("advanced" => $mysidia->input->post("dateunit")), "varname='duration'");
            }
            return;
        }
