    ...
    public function upload()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if (empty($hashedfilename)) {
                throw new InvalidIDException("file_notexist");
            }
            $existname = $uploaddir."/".$hashedfilename;
            if (file_exists($existname)) {
                throw new DuplicateIDException("file_exist");
            }
            if ($filesize > 156000) {
                throw new UnsupportedFileException("file_size");
            }
            ...
            $ffn = secure($ffn);
            if (empty($ffn)) {
                throw new UnsupportedFileException("Unknown image");
            }
    ...
    public function delete()
    {
        ...
        if (!$mysidia->input->post("iid")) {
            ...
            if (!is_writable($serverpath)) {
                throw new NoPermissionException("notwritable");
            }
            unlink($serverpath);
