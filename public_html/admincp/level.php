...
class ACPLevelController extends AppController
{
        ...
        if (!$mysidia->input->get("type") and !$mysidia->input->post("type")) {
            ...
            if ($adopt = $stmt->fetchObject()) {
                if (!is_numeric((string)$mysidia->input->get("level"))) {
                    ...
                    if (!$level->alternateimage) {
                        $level->alternateimage = "templates/icons/no.gif";
                    }
            ...
            if ($level = $stmt->fetchObject()) {
                ...
                if ($primimg and $primimg != "none") {
                    $mysidia->db->update("levels", array("primaryimage" => $primimg), "adoptiename='{$mysidia->input->get("type")}' AND thisislevel='{$mysidia->input->get("level")}'");
                }
                if ($altimg and $altimg != "none") {
                    $mysidia->db->update("levels", array("alternateimage" => $altimg), "adoptiename='{$mysidia->input->get("type")}' AND thisislevel='{$mysidia->input->get("level")}'");
                }
                if ($reqclicks) {
                    $mysidia->db->update("levels", array("requiredclicks" => $reqclicks), "adoptiename='{$mysidia->input->get("type")}' AND thisislevel='{$mysidia->input->get("level")}'");
                }
            } else {
                throw new InvalidIDException("global_id");
            }
        ...
        if ($mysidia->input->post("submit")) {
            ...
            foreach ($settings as $name) {
                if ($mysidia->input->post($name) != ($this->settings->{$name})) {
                    $mysidia->db->update("levels_settings", array("value" => $mysidia->input->post($name)), "name='{$name}'");
                }
            }
            return;
        }
        ...
        if ($mysidia->input->post("submit")) {
            ...
            foreach ($settings as $name) {
                if ($mysidia->input->post($name) != ($daycareSettings->$name)) {
                    $mysidia->db->update("daycare_settings", array("value" => $mysidia->input->post($name)), "name='{$name}'");
                }
            }
            return;
        }
    ...
    private function dataValidate($i)
    {
        ...
        if ($altHosted[$i] and $altLocal[$i] != "none") {
            throw new BlankFieldException("alt_image");
        }
