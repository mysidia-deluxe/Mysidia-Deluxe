...
class ACPAdoptController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            } elseif (!$mysidia->input->post("imageurl") and $mysidia->input->post("existingimageurl") == "none") {
            ...
            if ($mysidia->input->post("cba") == "conditions") {
                if ($mysidia->input->post("freqcond") == "enabled" and !is_numeric($mysidia->input->post("number"))) {
                    throw new BlankFieldException("condition_freq");
                }
                if ($mysidia->input->post("datecond") == "enabled" and !$mysidia->input->post("date")) {
                    throw new BlankFieldException("condition_date");
                }
                if ($mysidia->input->post("adoptscond") == "enabled") {
                    if (!$mysidia->input->post("moreless") or !is_numeric($mysidia->input->post("morelessnum")) or !$mysidia->input->post("levelgrle") or !is_numeric($mysidia->input->post("grlelevel"))) {
                        throw new BlankFieldException("condition_moreandlevel");
                    }
                }

                if ($mysidia->input->post("maxnumcond") == "enabled" and !is_numeric($mysidia->input->post("morethannum"))) {
                    throw new BlankFieldException("maxnum");
                }
                if ($mysidia->input->post("usergroupcond") == "enabled" and !is_numeric($mysidia->input->post("usergroups"))) {
                    throw new BlankFieldException("group");
                }
            }

            if ($mysidia->input->post("alternates") == "enabled") {
                if (!is_numeric($mysidia->input->post("altoutlevel")) or !is_numeric($mysidia->input->post("altchance"))) {
    ...
    public function edit()
    {
        ...
        if (!$mysidia->input->post("choose")) {
            ...
            if ($adopt->getWhenAvailable() != "always" and $adopt->getWhenAvailable() != "") {
                $availtext = "<b>This adoptable currently has adoption restrictions on it.</b>";
            } else {
                $availtext = "This adoptable currently does not have adoption restrictions on it.";
            }
