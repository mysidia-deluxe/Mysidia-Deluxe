...
class ACPBreedingController extends AppController
{
        ...
        if ($mysidia->input->post("submit")) {
            ...
            foreach ($settings as $name) {
                if ($mysidia->input->post($name) != ($breedingSettings->$name)) {
                    $mysidia->db->update("breeding_settings", array("value" => $mysidia->input->post($name)), "name='{$name}'");
                }
            }
            return;
        }
    ...
    {
        $mysidia = Registry::get("mysidia");
        if (!$mysidia->input->post("offspring")) {
            throw new BlankFieldException("offspring");
        }
        if (!$mysidia->input->post("parent") and !$mysidia->input->post("mother") and !$mysidia->input->post("father")) {
            throw new BlankFieldException("parent");
        }
        if ($mysidia->input->post("parent") and ($mysidia->input->post("mother") or $mysidia->input->post("father"))) {
