    ...
    public function edit()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if (is_numeric($mysidia->input->post("level"))) {
                $mysidia->db->update("users", array("usergroup" => $mysidia->input->post("level")), "uid='{$mysidia->input->get("uid")}'");
            }
            
            //Carry out user banning options
            if ($mysidia->input->post("canlevel") == "no") {
                $mysidia->db->update("users_status", array("canlevel" => 'no'), "uid='{$mysidia->input->get("uid")}'");
            }
            ...
            if ($mysidia->input->post("canfriend") == "no") {
                $mysidia->db->update("users_status", array("canfriend" => 'no'), "uid='{$mysidia->input->get("uid")}'");
            }
            if ($mysidia->input->post("cantrade") == "no") {
                $mysidia->db->update("users_status", array("cantrade" => 'no'), "uid='{$mysidia->input->get("uid")}'");
            }
            if ($mysidia->input->post("canbreed") == "no") {
                $mysidia->db->update("users_status", array("canbreed" => 'no'), "uid='{$mysidia->input->get("uid")}'");
            }
            if ($mysidia->input->post("canpound") == "no") {
                $mysidia->db->update("users_status", array("canpound" => 'no'), "uid='{$mysidia->input->get("uid")}'");
            }
            if ($mysidia->input->post("canshop") == "no") {
                $mysidia->db->update("users_status", array("canshop" => 'no'), "uid='{$mysidia->input->get("uid")}'");
            }
