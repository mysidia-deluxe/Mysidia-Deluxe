...
class ACPAdsController extends AppController{
	...
	public function edit(){
		...
		elseif($mysidia->input->post("submit")){
			...
			if($mysidia->input->post("aimp") >= $mysidia->input->post("impressions") and $mysidia->input->post("impressions") != 0) $mysidia->db->update("ads", array("status" => 'inactive'), "id='{$mysidia->input->get("aid")}'");
			else $mysidia->db->update("ads", array("status" => 'active'), "id='{$mysidia->input->get("aid")}'");
		    return;
		}
		else{
		    $ad = $mysidia->db->select("ads", array(), "id = '{$mysidia->input->get("aid")}' LIMIT 1")->fetchObject();			
		    if(!is_object($ad)) throw new InvalidIDException("nonexist");			
